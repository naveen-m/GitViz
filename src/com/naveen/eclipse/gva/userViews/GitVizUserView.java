...
/**
 *     GIT VIZ ADVANTAGE - v1.0a
 * This class is the basic VIEW in Eclipse workbench.
 * Basic features are configured in plugin.xml
 ...
 * @date 05/16/2015 (v0.5) 05/23(v0.6) 05/30(v0.7) 06/07(v0.8) 06/08(v0.9)
 * @date 06/08/2015 (v1.0a)
 * 
 * NOTE: the 'GitViz View' (from Eclipse Menu > View) ..... it is not a simple 'View' as in MVC. 
 * That is a completely different kind of 'View'.
 */
public class GitVizUserView extends ViewPart {
	...
	//protected DataViewComponentOldVersion1 mDataViewComponent = new DataViewComponentOldVersion1();

	// COMPONENT access to the DATA VIEW set of functionality
	protected DataViewComponent mDataViewComponent = new DataViewComponent();
	 ...
	 * 
	 * Bundle Startup-time: configures the VIEW and BUNDLE application
	 */
		...
		doubleClickAction = new Action() {
			...
			public void run() {
				...
				if (UITextContants.TASK_1_REPOSITORYFILES.equalsIgnoreCase(publishedSelectionElement.toString())) {
					...
					// Step 1. call the DATAVIEW component
					GitDataResponse dataResponse = mDataViewComponent.requestGitData(gdr, pte);

					// Step 2. process data response in the GUI
					processGUIResponse(dataResponse, GitDataRequest.RESPOSITORY_FILES);
				...
				// TASK #2: Given a File, get the CommitHistory (Revisions list)	
					...
					// Step 2. process data response in the GUI
					processGUIResponse(dataResponse, GitDataRequest.FILEHISTORY);
	...
	private void hookContextMenu() {
		...
		menuMgr.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager manager) {
				GitVizUserView.this.fillContextMenu(manager);
			}
		});
